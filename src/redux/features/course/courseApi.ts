...
export const courseApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    createCourse: builder.mutation<ApiResponse<Course>, Partial<Course>>({
      ...
      invalidatesTags: (result) => {
        const baseTags = result?.data?._id
          ? [
              { type: "Course" as const },
              { type: "Course" as const, id: result.data._id },
            ]
          : [{ type: "Course" as const }];
        ...
        const latestTag = result?.data?.publisher?._id
          ? [
              {
                type: "Course" as const,
                id: `LATEST-${result.data.publisher._id}`,
              },
            ]
          : [];
    ...
    saveCurriculum: builder.mutation<
      ApiResponse<null>,
      { courseId: string; sections: any[] }
    >({
    ...
    >({
      ...
      transformResponse: (
        response: PaginatedCourses & { success: boolean }
      ) => {
        return {
          page: response.page,
