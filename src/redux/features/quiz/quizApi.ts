...
import { QuestionData, Quiz } from "../../../types/quizType";
...
type RootState = {
  auth: { token: string | null }; // <-- ĐÚNG tên slice và field
};
const SERVER =
  process.env.EXPO_PUBLIC_API_URL || // Expo RN
  process.env.API_URL || // fallback nếu bạn build khác
  "http://192.168.1.155:8000/api";
...
export const quizApi = createApi({
  ...
  endpoints: (builder) => ({
    getAllQuizzes: builder.query<
      ApiResponse<Quiz[]>,
      { courseId?: string; difficulty?: string }
    >({
      query: (params) => ({
        url: "",
        method: "GET",
